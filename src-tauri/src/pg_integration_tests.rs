...
async fn duplicate_and_unnamed_columns_are_not_dropped() {
    ...
    let anonymous = run_query_impl(&pool, "SELECT 1, 2").await.unwrap();
...
async fn a_trailing_line_comment_does_not_break_the_wrapper() {
    ...
    let pool = pool(&config).await;
    let result = run_query_impl(&pool, "SELECT 1 AS a -- trailing comment").await.unwrap();
...
async fn numeric_precision_survives_the_round_trip() {
    ...
    let pool = pool(&config).await;
    let result = run_query_impl(&pool, "SELECT 12345678901234567890.123456789::numeric").await.unwrap();
...
async fn char_length_and_json_string_values_survive_an_edit() {
    ...
    let verify = run_query_impl(&pool, "SELECT tag, payload FROM ozendb_it_types.t WHERE id = 1")
        .await
        .unwrap();
...
#[tokio::test]
async fn a_read_only_connection_is_enforced_by_the_server_not_just_by_which_pool_method_is_called() {
    // Regression: run_pg_query wraps arbitrary SQL in a subquery, which stops
    // statements that can't live in a FROM clause (INSERT/UPDATE/DELETE/DDL) but
    // not a volatile function usable inside a SELECT — `nextval()` on a fresh
    // sequence advanced it even though this connection is read_only. The fix is
    // server-side (`default_transaction_read_only`, see pg_uri::options_for),
    // so it holds regardless of how a future caller reaches the driver.
    ...
    let err = run_query_impl(&read_only_pool, "SELECT nextval('ozendb_it_seq')").await.unwrap_err();
    // A server-rejected statement classifies as "command" (see postgres_code),
    // not "postgres" — the point of this test is that it's rejected at all.
